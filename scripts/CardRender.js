...
// Main function for local execution. Skipped for API
function main() {

  const history = JSON.parse(fs.readFileSync('./scripts/history/history.json', 'utf8'));
  const latest = history.at(-1);

  for (const [typeName, typeConfig] of Object.entries(CARD_CONFIG)) {
...
// Main build function, exported for API use.
export function buildSVG(item, itemConfig, username = USERNAME) {
  const { title, medal = 'STARTING' } = item;

  const ranked = rankStats(item, itemConfig.stats);
